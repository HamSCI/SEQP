...
class filter(object):
    ...
    def __init__(self, vtsig, numtaps=None, cutoff_low=None, cutoff_high=None,
                 width=None, window='blackman', pass_zero=True, scale=False,
                 newSigName='filtered'):
    ...
    def plotTransferFunction(self, xmin=0, xmax=None, ymin_mag=-150,
                             ymax_mag=5, ymin_phase=-180., ymax_phase=180.,
                             worN=None,fig=None):
        ...
        w = w / max(w) * self.nyq
        ax.plot(w, h_dB, '.-')
        # ax.axvline(x=self.fMax,color='r',ls='--',lw=2)

        ax.set_xlim(xmin,xmax)
        ax.set_ylim(ymin_mag,ymax_mag)

        xts  = ax.get_xticks()
        xtls = []
        for xt in xts:
            freq_str    = '{:.2f}'.format(xt)

            if xt != 0:
                per     = (1./xt) / (60.)
                per_str = '{:.0f}'.format(per)
            else:
                per_str = 'Inf'
        ...
        ax.set_xlim(xmin,xmax)
        ax.set_ylim(ymin_phase,ymax_phase)
